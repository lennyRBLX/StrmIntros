...
const strmIntrosCommandSource = {
    ...
    executeCommand: function(command, items) {
        ...
        if (command.startsWith('delver_')) {
            ...
            if (itemId && itemName) {
                return require(['components/strmintros/strmintros']).then(responses => {
...
setTimeout(() => {
    Emby.importModule('./modules/common/globalize.js').then(globalize => {
        strmIntrosCommandSource.globalize = globalize;
        Emby.importModule('./modules/common/itemmanager/itemmanager.js').then(itemmanager => {
            itemmanager.registerCommandSource(strmIntrosCommandSource);
        });
    });
}, 3000);
    ";
